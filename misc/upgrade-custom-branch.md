...
它采用“一个长期维护分支、一个不可变发布 tag”的方式：`tz-null-fix` 是可 rebase 的工作分支；每个已验证版本以 `vX.Y.Z-tz-null-fix` Git tag、GitHub Release 和 Docker tag 发布。旧的 `v3.0.2-tz-null-fix` 分支只保留作历史参考。
...
随后创建并推送发布 tag。该 tag 会触发镜像构建和 GitHub Release：
...
构建成功后，工作流会在 fork 中创建同名 GitHub Release。Immich 自建镜像的更新检测从该 Release 读取最新 tag。构建状态可在 [Actions 工作流页面](https://github.com/yrnyn/immich/actions/workflows/build-custom-server.yml) 查看。
...
对应的 Git tag 和 GitHub Release 也会继续保留。数据库迁移后的降级不受此流程保证，升级前仍应备份数据库。
