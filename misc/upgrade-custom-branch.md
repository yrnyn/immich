...
自定义版本检查及配套配置、测试已移除，`server/Dockerfile` 与官方版本一致。应用版本保持官方 `package.json` 中的版本号（当前 `3.2.2`），更新通知查询官方版本服务，关于页面恢复官方行为。自定义镜像是否可用以本仓库 Actions 结果及镜像 tag 为准；每个版本的 GitHub Release 会记录该次构建的镜像 digest，可用于追溯部署环境实际运行的构建。

历史上曾有一组"自定义版本检查"提交，已在 v3.1.0 → v3.2.0 的升级中弃用。后续升级继续核对上游业务代码的差异是否仅限日期处理，避免重新引入。
...
`.github/workflows/check-upstream-release.yml` 每天预演一次"把定制提交 rebase 到上游最新正式版"，
有新版本时把结论写进一个标题为 `跟进上游 vX.Y.Z` 的 GitHub Issue。无新版本时几秒内退出，不建 Issue。

这个工作流**完全只读**：不推分支、不打 tag、不构建镜像。它只负责把"上游更新了吗、这次跟进风险多大"
从一次人工调查变成一条通知；**要不要跟进由人决定**。

原因写在工作流文件开头，这里只记结论：Issue 里的"补丁漂移 ✅ 无漂移"只证明补丁**文本**没被 rebase
改写，不证明它对**改动后的上游代码**仍然语义正确。时区补丁改的是日期/时区处理逻辑，上游若重构了附近
代码，补丁可能应用干净、编译通过、行为却是错的。所以自动发布是不可接受的——推发布 tag 会一路走到移动
`v3`，等于自动上生产。
...
| 检查 | 命中时要做什么 |
| --- | --- |
| rebase 冲突 | 手工 rebase 并解决冲突，见"发生 rebase 冲突时" |
| 补丁漂移 | rebase 改写了补丁内容，逐行核对改写是否正确 |
| 上游改动定制文件 | **最需要注意的一项**：看上游那次改动和定制补丁在语义上是否冲突 |
| 部署文件 `docker/` 有变动 | 升级脚本会提示重新生成 custom compose；部署端必须覆盖 `docker-compose.yml` |
| Compose 同步规则失配 | 上游改了 compose 结构，同步规则本身要更新（见下节，三处） |
| 新数据库迁移 | 升级前备份数据库，降级不受保证 |

### 收到 Issue 之后

工作流**只做检测和评估，不做任何改动**。后续步骤和没有它时完全一样，区别只是不用自己盯上游、
也不用每次手工做一遍风险评估。完整闭环：

1. **读 Issue 的检查项表**。全绿直接进第 3 步。
2. **有 ⚠️ 时先人工核对**。最常见的是"上游改动定制文件"，核对方法：

   ```powershell
   git fetch upstream
   # 上游这次对补丁所在文件做了什么
   git log --oneline <基线>..<目标> -- server/src/services/metadata.service.ts
   git show <上面列出的 commit> -- server/src/services/metadata.service.ts
   # 对照定制补丁在该文件上的全部改动
   git diff <基线>..HEAD -- server/src/services/metadata.service.ts
   ```

   判断的是两者**是否碰同一段逻辑**。改的是不相干的功能、行号相隔很远，就可以继续；
   如果上游重构了补丁依赖的那段代码，即使 rebase 干净、range-diff 无漂移，也要重新
   审视补丁是否仍然正确——这正是这一步不能自动化的原因。
3. **本地跑升级脚本**（和以前完全一样）：

   ```powershell
   .\misc\upgrade-custom-branch.ps1 -TargetTag <目标>
   ```

   若 Issue 报了"部署文件 `docker/` 有变动"，脚本会在 rebase 后提示重新生成
   `docker-compose.custom.yml`；同意后按它给出的命令把改动折进部署提交：

   ```powershell
   git add docker/docker-compose.custom.yml
...
4. **推分支与发布 tag**：
   ...
   git tag -a <目标>-tz-null-fix -m "Release <目标>-tz-null-fix"
   git push origin <目标>-tz-null-fix
   ```
5. **等待自动发布**：构建镜像 → 发 Release 及附件 → 移动 `v3`（见"脚本流程与确认点"）。
6. **更新部署环境**：见"部署端约定"，一条命令。
7. **关闭 Issue**。下一个上游版本会另开一个。

只有第 2、3、4 步需要人；第 1、5 步自动，第 6 步一条命令。

两个维护注意点：

- GitHub 的 `schedule` **只从仓库默认分支触发**。本 fork 的默认分支是 `tz-null-fix`，所以能正常跑；
  若改回 `main`，这个工作流会**静默停止**（不报错），需一并同步或改回。
- 想立刻验证可在 Actions 里手动运行。`target_tag` 留空时行为和定时运行一致（只在上游比基线新时才继续）；
  **填了 tag 则跳过新旧判断**，无论新旧都跑完整预演并创建 Issue —— 填当前基线版本就是验证工作流本身的办法。
...
- CI：`build-custom-server.yml` 的 `release` job 发布前再校验一次，不一致直接失败，不会把过期的部署文件发出去。
- 预演：`check-upstream-release.yml` 在上游发新版时提前告知该规则是否仍然成立。

规则定义分散在这三个文件里，改动时必须**三处同步**。

想额外定制 Compose 时，优先考虑能否放进服务器端的 `docker-compose.override.yml`（见下节）。只有在 override
无法表达时才扩大仓库内的差异，并同时更新上述三处的同步规则。
